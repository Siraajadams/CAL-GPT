...
const defaultProfile = {
  ...
  exerciseHistory: "No regular exercise", eatingHabits: "Balanced meals most days", waterIntake: "", sleepAverage: "7", consent: false,
};
...
export default function Page() {
  ...
  const [aiStatus, setAiStatus] = useState("OpenAI Vision ready once /api/analyze and OPENAI_API_KEY are configured.");
  ...
  useEffect(() => {
    ...
    if (saved) { const data = JSON.parse(saved); setProfile(data.profile || defaultProfile); setMeals(data.meals || []); setWeights(data.weights || []); setSleepRecords(data.sleepRecords || []); setScreen("dashboard"); }
  }, []);
  ...
  function saveEHR(nextProfile = profile, nextMeals = meals, nextWeights = weights, nextSleep = sleepRecords) {
    localStorage.setItem("calgpt_ehr", JSON.stringify({ profile: nextProfile, meals: nextMeals, weights: nextWeights, sleepRecords: nextSleep }));
  }
  ...
  function submitEnrollment() {
    ...
    const firstSleep = [{ date: today, hours: Number(profile.sleepAverage || 0) }];
    setProfile(nextProfile); setWeights(firstWeight); setSleepRecords(firstSleep); saveEHR(nextProfile, meals, firstWeight, firstSleep); setScreen("dashboard");
  }
  function loginUser() {
    const saved = JSON.parse(localStorage.getItem("calgpt_ehr") || "null");
    if (saved?.profile?.email === login.email && saved?.profile?.password === login.password) { setProfile(saved.profile); setMeals(saved.meals || []); setWeights(saved.weights || []); setSleepRecords(saved.sleepRecords || []); setScreen("dashboard"); }
  ...
  async function handleImage(file: File | undefined) {
    ...
    reader.onload = async () => {
      ...
      setAiStatus("Analyzing image with OpenAI Vision...");
      try {
        const res = await fetch("/api/analyze", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ image: imageBase64, notes: mealForm.notes }) });
        if (!res.ok) throw new Error("API not configured yet");
        const data = await res.json();
        setMealForm((p) => ({ ...p, calories: String(data.calories || p.calories), notes: data.description || p.notes, foodGroups: data.foodGroups || estimateFoodGroups(data.description || p.notes) }));
        setAiStatus("OpenAI Vision analysis completed.");
      } catch {
        setMealForm((p) => ({ ...p, foodGroups: estimateFoodGroups(p.notes) }));
        setAiStatus("AI endpoint not connected yet. Using local food group interpreter for MVP.");
      }
    };
  ...
  function addMeal() {
    ...
    const nextMeals = [record, ...meals]; setMeals(nextMeals); saveEHR(profile, nextMeals, weights, sleepRecords);
  }
  function updateWeight() {
    ...
    setWeights(nextWeights); setProfile(nextProfile); saveEHR(nextProfile, meals, nextWeights, sleepRecords);
  }
  function addSleep() {
    const hours = prompt("Average sleep last night in hours"); if (!hours) return;
    const nextSleep = [...sleepRecords, { date: today, hours: Number(hours) }];
    setSleepRecords(nextSleep); saveEHR(profile, meals, weights, nextSleep);
  }
