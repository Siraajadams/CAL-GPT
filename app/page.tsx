...
const sleepOptions = [
  ...
  "5 to 7 hours per night",
  "Over 8 hours per night",
];
...
export default function Page() {
  ...
  const [aiStatus, setAiStatus] = useState(
    "Upload a meal photo, then tap Submit image for AI analysis.",
  );
  ...
  const avgDaily = meals.length
    ? Math.round(
        totalCalories / Math.max(1, new Set(meals.map((m) => m.date)).size),
      )
    : 0;
  const avgSleepHours = sleepRecords.length
    ? (
        sleepRecords.reduce((s, r) => s + sleepCategoryToHours(r.hours), 0) /
        sleepRecords.length
      ).toFixed(1)
  ...
  function submitEnrollment() {
    if (
      ...
      return alert(
        "Please complete name, DOB, email, password, mobile and consent.",
      );
    ...
    const firstWeight = [
      {
        ...
        bmi: Number(calcBMI(profile.weight, profile.height) || 0),
      },
    ];
    const firstSleep = [
      {
        date: today,
        hours: profile.sleepAverage,
        value: sleepCategoryToHours(profile.sleepAverage),
      },
    ];
  ...
  function resetPassword() {
    ...
    if (!saved?.profile?.email)
      return alert("No local EHR profile found on this device.");
    if (!resetEmail || !newPassword)
      return alert("Enter your registered email and a new password.");
    if (saved.profile.email.toLowerCase() !== resetEmail.toLowerCase())
      return alert("Email does not match the saved profile on this device.");
  ...
  async function submitImageForAI() {
    try {
      ...
      const imageBlob = await fetch(mealForm.imageBase64).then((r) => r.blob());
      const formData = new FormData();
      formData.append("image", imageBlob, mealForm.imageName || "meal.jpg");
  ...
  function updateWeight() {
    ...
    const nextWeights = [
      ...weights,
      {
        ...
        bmi: Number(calcBMI(newWeight, profile.height) || 0),
      },
    ];
  ...
  function addSleep() {
    const hours = prompt(
      "Select sleep category: Less than 5 hours per night, 5 to 7 hours per night, or Over 8 hours per night",
      profile.sleepAverage,
    );
    if (!hours) return;
    const nextSleep = [
      ...sleepRecords,
      { date: today, hours, value: sleepCategoryToHours(hours) },
    ];
    const nextProfile = { ...profile, sleepAverage: hours };
    setSleepRecords(nextSleep);
    setProfile(nextProfile);
    saveEHR(nextProfile, meals, weights, nextSleep, activityUpdates);
  }
  function updateActivity() {
    const activity = prompt(
      "Update activity level or exercise history",
    ...
    setActivityUpdates(nextActivity);
    setProfile(nextProfile);
    saveEHR(nextProfile, meals, weights, sleepRecords, nextActivity);
  }
  ...
  const sleepGraph = () => {
    const clean = sleepRecords
      .map((r) => ({ ...r, value: sleepCategoryToHours(r.hours) }))
      .filter((r) => r.value > 0)
      .slice(-7);
    if (!clean.length) return <p>No tracking data yet.</p>;
  ...
  return (
    <main
      style={{
        minHeight: "100vh",
        background:
          "radial-gradient(circle at top left,#bbf7d0,#f1f5f9 35%,#e0f2fe)",
        color: "#0f172a",
      ...
      <div style={{ maxWidth: 480, margin: "0 auto", paddingBottom: 94 }}>
        {screen === "landing" && (
          ...
          >
            ...
            <p style={{ color: "#bbf7d0", fontWeight: 800 }}>
              AI nutrition EHR and calorie tracker
            </p>
            <div
              style={{
                ...cardStyle,
            ...
            >
              <h2 style={{ fontSize: 36, lineHeight: 1.05 }}>
                Fight food noise. Build healthier habits.
              </h2>
              <p style={{ color: "#e2e8f0", marginTop: 16 }}>
                Capture meals, identify food groups, track BMI, sleep, weight
                and generate patient progress reports.
              </p>
              <div
                style={{
                  display: "grid",
              ...
              <button
                style={{ ...buttonStyle, marginTop: 28 }}
            ...
            <div style={{ ...cardStyle, color: "#0f172a" }}>
              <h2>About us</h2>
              <p>
                CalGPT was built for people struggling to fight the food noise,
                trying to eat healthier, and hopefully lose weight in a
                practical and supportive way. It was developed by a team of
                doctors and pharmacists to create a clinical wellness tool for
                patients who want to improve their health, reduce diabetes and
                cholesterol risk, and build better daily habits through meal
                tracking, weight monitoring and structured nutrition support.
              </p>
            </div>
          </section>
        )}
        ...
        {screen === "login" && (
          <section style={{ padding: 24 }}>
            ...
            <div style={cardStyle}>
              ...
              <button
                style={{ ...buttonStyle, marginTop: 12, background: "#0f172a" }}
                onClick={() => setScreen("reset")}
              >
                Forgot / reset password
              </button>
        ...
        {screen === "reset" && (
          <section style={{ padding: 24 }}>
            ...
            <p>
              This MVP resets the password for the saved local EHR profile on
              this device.
            </p>
            <div style={cardStyle}>
              <label style={labelStyle}>
        ...
        {screen === "enroll" && (
          <section style={{ padding: 20 }}>
            ...
            <div style={cardStyle}>
              ...
              <label style={labelStyle}>
                Dialling code
                <input style={inputStyle} value={profile.dialCode} readOnly />
              </label>
              <label style={labelStyle}>
                Mobile number
                <input
              ...
              <label style={labelStyle}>
                Sleep per night average
                <select
                  style={inputStyle}
                  value={profile.sleepAverage}
                  onChange={(e) =>
                    setProfile({ ...profile, sleepAverage: e.target.value })
                  }
                >
                  {sleepOptions.map((x) => (
              ...
              <label style={labelStyle}>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setProfile({ ...profile, exerciseHistory: e.target.value })
                  }
                >
              ...
              <label style={labelStyle}>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setProfile({ ...profile, eatingHabits: e.target.value })
                  }
                >
                  {eatingHabitOptions.map((x) => (
                    <option key={x}>{x}</option>
                  ))}
                </select>
              </label>
              ...
              <label style={labelStyle}>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setProfile({ ...profile, waterIntake: e.target.value })
                  }
                >
                  {waterOptions.map((x) => (
                    <option key={x}>{x}</option>
              ...
              <label style={{ display: "flex", gap: 10, marginBottom: 18 }}>
                ...
                />
                I consent to storing my health profile, meal images and calorie
                records.
              </label>
              <label style={labelStyle}>
                Create password
              ...
              <button style={buttonStyle} onClick={submitEnrollment}>
                Submit enrolment
              </button>
            </div>
          </section>
        ...
        {screen === "dashboard" && (
          <section style={{ padding: 20 }}>
            <div
              style={{
                ...cardStyle,
                background: "linear-gradient(135deg,#061115,#064e3b)",
                color: "white",
              }}
            >
              ...
              <p>
                {profile.goal} • BMI {bmi || "--"} • Sleep: {avgSleep}
              </p>
              <button
                style={{
                  ...buttonStyle,
            ...
            >
              <div style={cardStyle}>
                ...
                <h2 style={{ fontSize: 40, color: "#22c55e" }}>
                  {todayCalories}
                </h2>
                <p>kcal</p>
              </div>
              <div style={cardStyle}>
                <p>Meals Today</p>
                <h2 style={{ fontSize: 40 }}>{todayMeals.length}</h2>
                <p>records</p>
              </div>
            </div>
            <div style={cardStyle}>
              ...
              {todayMeals.length === 0 ? (
                ...
                todayMeals.map((m) => (
                  ...
                  >
                    <b>{m.category}</b> • {m.calories} kcal
                    <br />
                    <small>
                      {m.time} • {m.foodGroups?.join(", ")}
                    </small>
                  </div>
                ))
              )}
        ...
        {screen === "ehr" && (
          <section style={{ padding: 20 }}>
            ...
            <div style={cardStyle}>
              ...
              <b>DOB:</b> {profile.dob}
              <br />
              <b>Mobile:</b> {profile.dialCode} {profile.mobile}
              <br />
              <b>Address:</b> {profile.address}, {profile.country}
            </div>
            <div style={cardStyle}>
              <b>Conditions:</b> {profile.conditions || "None recorded"}
              <br />
              <b>Medication:</b> {profile.medications || "None recorded"}
              <br />
              <b>Allergies:</b> {profile.allergies || "None recorded"}
              <br />
              <b>Exercise:</b> {profile.exerciseHistory}
              <br />
              <b>Eating habits:</b> {profile.eatingHabits}
              <br />
            ...
            <div style={cardStyle}>
              ...
              <p>Date-stamped longitudinal weight changes.</p>
              {barGraph(
                weights,
                "weight",
            ...
            <div style={cardStyle}>
              ...
              <p>
                Average sleep per night: <b>{avgSleep}</b>
              </p>
              {sleepGraph()}
              <label style={labelStyle}>
                Update sleep
                  ...
                  onChange={(e) => {
                    ...
                    const nextSleep = [
                      ...sleepRecords,
                      {
                        date: today,
                        hours,
                        value: sleepCategoryToHours(hours),
                      },
                    ];
                    const nextProfile = { ...profile, sleepAverage: hours };
                    setProfile(nextProfile);
            ...
            <button style={buttonStyle} onClick={updateWeight}>
              Add weight update
            </button>
          </section>
        )}
        ...
        {screen === "scanner" && (
          <section style={{ padding: 20 }}>
            ...
            <div style={cardStyle}>
              ...
              >
                <b>Food group interpreter:</b>
                <br />
                {(mealForm.foodGroups.length
                  ? mealForm.foodGroups
        ...
        {screen === "planner" && (
          <section style={{ padding: 20 }}>
            ...
            <p>
              Based on {profile.goal}, BMI {bmi || "--"} and {profile.activity}{" "}
              activity.
            </p>
            {mealPlan.map((item) => (
              <div style={cardStyle} key={item.meal}>
                ...
                <h3>{item.food}</h3>
                <p>
                  <b>Portion:</b> {item.portion}
                </p>
                <p>
                  <b>Calories:</b> {item.kcal} kcal
                </p>
              </div>
            ))}
          </section>
          ...
          <section style={{ padding: 20 }}>
            ...
            <div style={cardStyle}>
              ...
              >
                ...
                <p>
                  The patient has logged {meals.length} meal records. Continue
                  structured meal logging, portion control, water intake
                  monitoring, sleep tracking and weekly weight updates.
                  Longitudinal BMI, weight and sleep graphs are available in the
                  EHR section.
                </p>
              </div>
              <button
                style={{ ...buttonStyle, marginTop: 16, background: "#0f172a" }}
